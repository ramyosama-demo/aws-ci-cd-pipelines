...
resource "aws_codebuild_project" "dev_tflint" {
  count          = var.enable_dev == true ? 1 : 0
  name           = "${var.repository_name}-dev-tflint"
  description    = "Managed using Terraform"
  service_role   = aws_iam_role.codebuild.arn
  encryption_key = data.aws_ssm_parameter.cmk_arn.value
  tags           = var.tags
...
resource "aws_codebuild_project" "dev_checkov" {
  count          = var.enable_dev == true ? 1 : 0
  name           = "${var.repository_name}-dev-checkov"
  description    = "Managed using Terraform"
  service_role   = aws_iam_role.codebuild.arn
  encryption_key = data.aws_ssm_parameter.cmk_arn.value
  tags           = var.tags
      ...
      "Statement" : [
        {
          ...
          "Resource" : [
            ...
            aws_codebuild_project.dev_tflint[0].arn,
            aws_codebuild_project.dev_checkov[0].arn,
